...
-- Add the new column
ALTER TABLE Eras ADD SplashScreen TEXT DEFAULT 'ERA_Ancient.dds';
...
UPDATE Eras 
	SET SplashScreen= 'ERA_Enlightenment.dds' 
		WHERE Type = 'ERA_ENLIGHTENMENT'
			AND EXISTS (SELECT * FROM Units WHERE TYPE = 'UNIT_EE_SKIRMISHER');

UPDATE Eras 
	SET SplashScreen = 'ERA_FW_Future.dds' 
		WHERE Type = 'ERA_FW_FUTURE'
			AND EXISTS (SELECT * FROM Units WHERE TYPE='UNIT_FW_SUBORBITAL_BOMBER');